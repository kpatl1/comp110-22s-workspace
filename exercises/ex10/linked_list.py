...
def value_at(head: Optional[Node], index: int) -> Optional[int]:
    """Returns the value of the given index in a linked list."""
    if head is None:
...
def linkify(items: list[int]) -> Optional[Node]:
    """Creates a linked list from a given list of type ints."""
    if len(items) == 0:
        return None
    else:
...
def scale(head: Optional[Node], factor: int) -> Optional[Node]:
    """Scales all data values in a given linked list by a given factor."""
    if head is None:
        return None
    else:
