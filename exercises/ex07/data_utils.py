...
def column_values(input_table: list[dict[str, str]], column_name: "str") -> list[str]:
    """Gets the values of a single column and results them in a single list."""
    result: list[str] = []
...
def head(data_table: dict[str, list[str]], n: int) -> dict[str, list[str]]:
    """Creates a column n rows long from a given input dictionary with type string to list[str]."""
    result: dict[str, list[str]] = {}
    for column in data_table:
        n_values_in_column: list[str] = []
        if n >= len(data_table[column]):
...
def select(data_table: dict[str, list[str]], column_subset_names: list[str]) -> dict[str, list[str]]:
    """Selects a subset of columns from a data table with given column names."""
    result: dict[str, list[str]] = {}
...
def count(count_values: list[str]) -> dict[str, int]:
    """Counts the number of given values in a list and returns it as a dictionary."""
    result: dict[str, int] = {}
